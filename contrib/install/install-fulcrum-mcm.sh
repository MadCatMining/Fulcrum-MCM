...
if [ -n "$PORT_SSL" ] || [ -n "$PORT_WSS" ]; then
    ...
    c_y "(The installer copies these into a fulcrum-owned dir so perms stay stable across LE renewals.)"
    ...
    if [ -z "$CERT_SRC" ] || [ -z "$KEY_SRC" ]; then
        ...
        # Detect a Let's Encrypt source (…/letsencrypt/live/<NAME>/…) to offer a renewal deploy-hook.
...
echo
yesno "Proceed with installation?" Y || { c_y "Aborted."; exit 0; }
